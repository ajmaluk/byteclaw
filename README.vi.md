...
🤖 **ByteClaw** là trợ lý AI cá nhân siêu nhẹ, lấy cảm hứng từ [nanobot](https://github.com/HKUDS/nanobot), được viết lại hoàn toàn bằng **Go** thông qua quá trình "tự khởi tạo" (self-bootstrapping) — nơi chính AI Agent đã tự dẫn dắt toàn bộ quá trình chuyển đổi kiến trúc và tối ưu hóa mã nguồn.
...
>
> * **KHÔNG CÓ CRYPTO:** ByteClaw **KHÔNG** có bất kỳ token/coin chính thức nào. Mọi thông tin trên `pump.fun` hoặc các sàn giao dịch khác đều là **LỪA ĐẢO**.
> * **DOMAIN CHÍNH THỨC:** Website chính thức **DUY NHẤT** là **[byteclaw.io](https://byteclaw.io)**, website công ty là **[ajmaluk.com](https://ajmaluk.com)**.
> * **Cảnh báo:** Nhiều tên miền `.ai/.org/.com/.net/...` đã bị bên thứ ba đăng ký, không phải của chúng tôi.
> * **Cảnh báo:** ByteClaw đang trong giai đoạn phát triển sớm và có thể còn các vấn đề bảo mật mạng chưa được giải quyết. Không nên triển khai lên môi trường production trước phiên bản v1.0.
> * **Lưu ý:** ByteClaw gần đây đã merge nhiều PR, dẫn đến bộ nhớ sử dụng có thể lớn hơn (10–20MB) ở các phiên bản mới nhất. Chúng tôi sẽ ưu tiên tối ưu tài nguyên khi bộ tính năng đã ổn định.


## 📢 Tin tức

2026-02-16 🎉 ByteClaw đạt 12K stars chỉ trong một tuần! Cảm ơn tất cả mọi người! ByteClaw đang phát triển nhanh hơn chúng tôi tưởng tượng. Do số lượng PR tăng cao, chúng tôi cấp thiết cần maintainer từ cộng đồng. Các vai trò tình nguyện viên và roadmap đã được công bố [tại đây](docs/ROADMAP.md) — rất mong đón nhận sự tham gia của bạn!

2026-02-13 🎉 ByteClaw đạt 5000 stars trong 4 ngày! Cảm ơn cộng đồng! Chúng tôi đang hoàn thiện **Lộ trình dự án (Roadmap)** và thiết lập **Nhóm phát triển** để đẩy nhanh tốc độ phát triển ByteClaw.  
...
2026-02-09 🎉 ByteClaw chính thức ra mắt! Được xây dựng trong 1 ngày để mang AI Agent đến phần cứng $10 với RAM <10MB. 🤖 ByteClaw, Lên Đường!
...
Tải file binary cho nền tảng của bạn từ [trang Release](https://github.com/ajmaluk/byteclaw/releases).
...
Bạn cũng có thể chạy ByteClaw bằng Docker Compose mà không cần cài đặt gì trên máy.
...
> [!TIP]
> **Người dùng Docker**: Theo mặc định, Gateway lắng nghe trên `127.0.0.1`, không thể truy cập từ máy chủ. Nếu bạn cần truy cập các endpoint kiểm tra sức khỏe hoặc mở cổng, hãy đặt `BYTECLAW_GATEWAY_HOST=0.0.0.0` trong môi trường của bạn hoặc cập nhật `config.json`.
...
```bash
# Đặt câu hỏi
docker compose -f docker/docker-compose.yml run --rm byteclaw-agent -m "2+2 bằng mấy?"
...
> [!TIP]
> Thiết lập API key trong `~/.byteclaw/config.json`.
...
**2. Cấu hình** (`~/.byteclaw/config.json`)
...
> `request_timeout` là tùy chọn và dùng đơn vị giây. Nếu bỏ qua hoặc đặt `<= 0`, ByteClaw sẽ dùng timeout mặc định (120s).
...
```bash
byteclaw agent -m "Xin chào, bạn là ai?"
```
...
Trò chuyện với ByteClaw qua Telegram, Discord, DingTalk, LINE hoặc WeCom.
...
> **Docker Compose**: Thêm `ports: ["18791:18791"]` vào service `byteclaw-gateway` để mở port webhook.
...
ByteClaw hỗ trợ hai loại tích hợp WeCom:
...
Kết nối ByteClaw với Mạng xã hội Agent chỉ bằng cách gửi một tin nhắn qua CLI hoặc bất kỳ ứng dụng Chat nào đã tích hợp.

**Đọc `https://clawdchat.ai/skill.md` và làm theo hướng dẫn để tham gia [ClawdChat.ai](https://clawdchat.ai)**

## ⚙️ Cấu hình chi tiết

File cấu hình: `~/.byteclaw/config.json`

### Cấu trúc Workspace

ByteClaw lưu trữ dữ liệu trong workspace đã cấu hình (mặc định: `~/.byteclaw/workspace`):
...
ByteClaw chạy trong môi trường sandbox theo mặc định. Agent chỉ có thể truy cập file và thực thi lệnh trong phạm vi workspace.
...
|----------|---------|-------|
| `workspace` | `~/.byteclaw/workspace` | Thư mục làm việc của agent |
...
ByteClaw có thể tự động thực hiện các tác vụ định kỳ. Tạo file `HEARTBEAT.md` trong workspace:
...
* `BYTECLAW_HEARTBEAT_ENABLED=false` để tắt
* `BYTECLAW_HEARTBEAT_INTERVAL=60` để thay đổi khoảng thời gian
...
> **Tính năng mới!** ByteClaw hiện sử dụng phương pháp cấu hình **đặt mô hình vào trung tâm**. Chỉ cần chỉ định dạng `nhà cung cấp/mô hình` (ví dụ: `zhipu/glm-4.7`) để thêm nhà cung cấp mới—**không cần thay đổi mã!**
...
```
> Chạy `byteclaw auth login --provider anthropic` để thiết lập thông tin xác thực OAuth.
...
Định cấu hình nhiều endpoint cho cùng một tên mô hình—ByteClaw sẽ tự động phân phối round-robin giữa chúng:
...
| Lệnh | Mô tả |
| --- | --- |
| `byteclaw onboard` | Khởi tạo cấu hình & workspace |
| `byteclaw agent -m "..."` | Trò chuyện với agent |
| `byteclaw agent` | Chế độ chat tương tác |
| `byteclaw gateway` | Khởi động gateway (cho bot chat) |
| `byteclaw status` | Hiển thị trạng thái |
| `byteclaw cron list` | Liệt kê tất cả tác vụ định kỳ |
| `byteclaw cron add ...` | Thêm tác vụ định kỳ |

### Tác vụ định kỳ / Nhắc nhở

ByteClaw hỗ trợ nhắc nhở theo lịch và tác vụ lặp lại thông qua công cụ `cron`:
...
Các tác vụ được lưu trong `~/.byteclaw/workspace/cron/` và được xử lý tự động.
...
Điều này là bình thường nếu bạn chưa cấu hình API key cho tìm kiếm. ByteClaw sẽ cung cấp các liên kết hữu ích để tìm kiếm thủ công.
...
Thêm key vào `~/.byteclaw/config.json` nếu dùng Brave:
...
Điều này xảy ra khi có một instance bot khác đang chạy. Đảm bảo chỉ có một tiến trình `byteclaw gateway` chạy tại một thời điểm.
