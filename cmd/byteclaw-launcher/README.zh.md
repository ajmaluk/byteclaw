...
ByteClaw 的独立启动器，提供可视化 JSON 配置编辑和 OAuth Provider 认证管理。
...
# 运行（使用默认配置路径 ~/.byteclaw/config.json）
./byteclaw-launcher
...
```
Usage: byteclaw-launcher [options] [config.json]

Arguments:
  config.json    配置文件路径（默认: ~/.byteclaw/config.json）
...
认证完成后 Google 回调至 `GET /auth/callback`，自动保存凭据并重定向回 byteclaw-config 页面。
