...
ByteClaw's tools configuration is located in the `tools` field of `config.json`.
...
By default, ByteClaw blocks the following dangerous commands:
...
All configuration options can be overridden via environment variables with the format `BYTECLAW_TOOLS_<SECTION>_<KEY>`:

For example:

- `BYTECLAW_TOOLS_WEB_BRAVE_ENABLED=true`
- `BYTECLAW_TOOLS_EXEC_ENABLE_DENY_PATTERNS=false`
- `BYTECLAW_TOOLS_CRON_EXEC_TIMEOUT_MINUTES=10`
- `BYTECLAW_TOOLS_MCP_ENABLED=true`
