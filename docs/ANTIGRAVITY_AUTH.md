...
**Antigravity** (Google Cloud Code Assist) is a Google-backed AI model provider that offers access to models like Claude Opus 4.6 and Gemini through Google's Cloud infrastructure. This document provides a complete guide on how authentication works, how to fetch models, and how to implement a new provider in ByteClaw.
...
10. [Creating a New Provider in ByteClaw](#creating-a-new-provider-in-byteclaw)
...
const antigravityPlugin = {
  ...
  register(api: ByteClawPluginApi) {
...
type ProviderAuthResult = {
  ...
  configPatch?: Partial<ByteClawConfig>;
  defaultModel?: string;
  notes?: string[];
};
```
...
- Go ≥ 1.21
- ByteClaw codebase (`pkg/providers/` and `pkg/auth/`)
...
Auth profiles are stored in `~/.byteclaw/auth.json`:
...
## Creating a New Provider in ByteClaw

ByteClaw providers are implemented as Go packages under `pkg/providers/`. To add a new provider:
...
If your provider requires OAuth or special authentication, add a case to `cmd/byteclaw/cmd_auth.go`:
...
byteclaw auth login --provider your-provider

# List models (for Antigravity)
byteclaw auth models

# Start the gateway
byteclaw gateway

# Run an agent with a specific model
byteclaw agent -m "Hello" --model your-model
```
...
export BYTECLAW_AGENTS_DEFAULTS_MODEL=your-model

# Override provider settings
export BYTECLAW_MODEL_LIST='[{"model_name":"your-model","model":"your-provider/model-name","api_key":"..."}]'
```
...
### "Token expired"
- Refresh OAuth tokens: `byteclaw auth login --provider antigravity`
...
- Check auth profile storage: `~/.byteclaw/auth.json`
- Re-run `byteclaw auth login --provider antigravity`
