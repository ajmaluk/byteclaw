...
🤖 ByteClaw は [nanobot](https://github.com/HKUDS/nanobot) にインスパイアされた超軽量パーソナル AI アシスタントです。Go でゼロからリファクタリングされ、AI エージェント自身がアーキテクチャの移行とコード最適化を推進するセルフブートストラッピングプロセスで構築されました。
...
## 📢 ニュース
2026-02-09 🎉 ByteClaw リリース！$10 ハードウェアで 10MB 未満の RAM で動く AI エージェントを 1 日で構築。🤖 行くぜ、シャコ！
...
Docker Compose を使えば、ローカルにインストールせずに ByteClaw を実行できます。
...
> [!TIP]
> **Docker ユーザー**: デフォルトでは、Gateway は `127.0.0.1` でリッスンしており、ホストからアクセスできません。ヘルスチェックエンドポイントにアクセスしたり、ポートを公開したりする必要がある場合は、環境変数で `BYTECLAW_GATEWAY_HOST=0.0.0.0` を設定するか、`config.json` を更新してください。
...
> [!TIP]
> `~/.byteclaw/config.json` に API キーを設定してください。
...
> `request_timeout` は任意の秒単位設定です。省略または `<= 0` の場合、ByteClaw はデフォルトのタイムアウト（120秒）を使用します。
...
Telegram、Discord、QQ、DingTalk、LINE、WeCom で ByteClaw と会話できます
...
> **Docker Compose**: `byteclaw-gateway` サービスに `ports: ["18791:18791"]` を追加して Webhook ポートを公開してください。
...
ByteClaw は2種類の WeCom 統合をサポートしています：
...
設定ファイル: `~/.byteclaw/config.json`

### ワークスペース構成

ByteClaw は設定されたワークスペース（デフォルト: `~/.byteclaw/workspace`）にデータを保存します：
...
ByteClaw はデフォルトでサンドボックス環境で実行されます。エージェントは設定されたワークスペース内のファイルにのみアクセスし、コマンドを実行できます。
...
| オプション | デフォルト | 説明 |
|-----------|-----------|------|
| `workspace` | `~/.byteclaw/workspace` | エージェントの作業ディレクトリ |
...
ByteClaw は自動的に定期タスクを実行できます。ワークスペースに `HEARTBEAT.md` ファイルを作成します：
...
**環境変数:**
- `BYTECLAW_HEARTBEAT_ENABLED=false` で無効化
- `BYTECLAW_HEARTBEAT_INTERVAL=60` で間隔変更
...
> **新機能！** ByteClaw は現在 **モデル中心** の設定アプローチを採用しています。`ベンダー/モデル` 形式（例: `zhipu/glm-4.7`）を指定するだけで、新しいプロバイダーを追加できます—**コードの変更は一切不要！**
...
```
> OAuth認証を設定するには、`byteclaw auth login --provider anthropic` を実行してください。
...
同じモデル名で複数のエンドポイントを設定すると、ByteClaw が自動的にラウンドロビンで分散します：
...
| コマンド | 説明 |
|---------|------|
| `byteclaw onboard` | 設定＆ワークスペースの初期化 |
| `byteclaw agent -m "..."` | エージェントとチャット |
| `byteclaw agent` | インタラクティブチャットモード |
| `byteclaw gateway` | ゲートウェイを起動 |
| `byteclaw status` | ステータスを表示 |
...
検索 API キーをまだ設定していない場合、これは正常です。ByteClaw は手動検索用の便利なリンクを提供します。
...
別のインスタンスが実行中の場合に発生します。`byteclaw gateway` が 1 つだけ実行されていることを確認してください。
